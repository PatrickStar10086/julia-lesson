...
# 约束条件
@constraint(model, [i in 1:N, g in 1:G], sum(X[i, g, 1:M[g]]) == 1) # 每个工件在每个阶段只能在一个机器上加工
@constraint(model, [i in 1:N,j in 1:N, g in 1:G], Y[i, j, g] + Y[j, i, g] <= 1) # 工件i与j的先后顺序
@constraint(model, [i in 1:N, j in 1:N, g in 1:G], B[i, g] - (B[j, g] + p[i, g]) + (U * (3 - Y[i, j, g] - X[i, g, m] - X[j, g, m]) for m in M[g]) >= 0) # 完工时间等于开始时间加工时间
@constraint(model, [i in 1:N, g in 1:G], B[i, g] >= 0) # 加工开始时间大于等于0
@constraint(model, [i in 1:N-1, g in 1:G], C[i+1, g] >= B[i, g] + p[i, g]) # 完工时间等于开始时间加工时间
